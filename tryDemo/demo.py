"""
use supported weights file 3附图
"""
import os
import sys
import importlib
...
class MainWindow(QWidget):
    ...
    def init_label(self):
        ...
        label_layout.addStretch(1)
        label2 = QLabel('组件分割预测图')
        label_layout.addWidget(label2)
        label_layout.addStretch(1)
        label5 = QLabel('组件分割实物图')
        label_layout.addWidget(label5)
        label_layout.addStretch(1)
        return label_layout
    ...
    def init_show(self):
        ...
        label4.setFixedSize(400, 400)
        label4.setText("")
        label_layout.addWidget(label4)
        self.label6 = label6 = QLabel()
        label6.setFixedSize(400, 400)
        label6.setText("")
        label_layout.addWidget(label6)
        return label_layout
    ...
    def button_seg_click(self):
        ...
        # 将save_path中的路径图片显示到标签
        # 通过文件路径获取图片文件，并设置图片长宽为label控件的长、宽
        img = QPixmap(predict_pcd).scaled(self.label4.width(), self.label4.height())
        #将参数显示在文本上
        self.text_edit.setText(str(args))
        # 在label控件上显示选择的图片
        ...
        lab_pcd = '/home/chen/Pointnet_Pointnet2_pytorch-master/data/shapenetcore_partanno_segmentation_benchmark_v0_normal/result/label_3d_img/0_None_img.png'
        # 将save_path中的路径图片显示到标签
        # 通过文件路径获取图片文件，并设置图片长宽为label控件的长、宽
        img = QPixmap(lab_pcd).scaled(self.label6.width(), self.label6.height())
        #将参数显示在文本上
        self.text_edit.setText(str(args))
        # 在label控件上显示选择的图片
        self.label6.setPixmap(img)
...
class PartNormalDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if dataset_name == 'shapenetcore_partanno_segmentation_benchmark_v0_normal':
            d = ' ' 
            # 从文件名拿到类名id
            cat_folder = os.path.dirname(self.img_root).split('/')[-1]  # 获取倒数第一个目录名 “02691156”
            #通过文件名在字典中找到对应的类名
            # 遍历字典的键  
