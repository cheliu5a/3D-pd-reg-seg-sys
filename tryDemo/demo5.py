...
class Generate_txt_and_3d_img:
    ...
    def draw_3d_img(self):
        ...
        each_txt_path = os.listdir(self.all_pred_txt_path) # 拿到所有预测后txt文件的路径
        ...
        for idx,(txt, lab) in tqdm.tqdm(enumerate(zip(each_txt_path, each_label)),total=len(each_txt_path)):
            ...
            # 选择的是shapenet数据集
            if self.dataset_name == 'shapenetcore_partanno_segmentation_benchmark_v0_normal':
                self.draw_each_img(os.path.join(lab_txt_path, lab),idx, save_path = lab_img_path, heat_maps = self.heat_map)
            # 可视化pre_txt
            self.draw_each_img(os.path.join(pre_txt_path, txt),idx, save_path = pre_img_path, heat_maps = self.heat_map)
