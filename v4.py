...
#matplotlib.use("Agg") 
matplotlib.use("Qt5Agg") #指定渲染后端。QtAgg后端指用Agg二维图形库在Qt控件上绘图。
...
class Child1Window(QDialog):
    ...
    def init_show(self):
        ...
        label3.setText("")
        '''
        pushButton = QPushButton(self.label3)
        pushButton_2 = QPushButton(self.label3)
        #按钮信号连接对象
        pushButton.clicked.connect(self.button)
        pushButton_2.clicked.connect(self.button_2)
        '''
        #声明在label3创建右键菜单
        self.label3.setContextMenuPolicy(Qt.CustomContextMenu)
        self.label3.customContextMenuRequested.connect(self.create_rightmenu)  # 连接到菜单显示函数
    ...
    def create_rightmenu(self):
        #菜单对象
        self.label3_menu = QMenu(self.label3)
        #self.actionA = QAction(QIcon('image/保存.png'),u'保存数据',self)

        self.actionC = QAction(u'查看大图',self)
        self.actionC.setShortcut('Ctrl+S')#设置快捷键
        self.label3_menu.addAction(self.actionC)
 
        self.actionC.triggered.connect(self.button_3)
 
        self.label3_menu.popup(QCursor.pos())#声明当鼠标在groupBox控件上右击时，在鼠标位置显示右键菜单   ,exec_,popup两个都可以，

    def button_3(self):# 点击button_check_click后
        plt.close("all") #this is the line to be added 只显示最后一张图
        fig = plt.figure()
        from mpl_toolkits.mplot3d import Axes3D  # 空间三维画图
        ax = Axes3D(fig)
        # 3D 轴（属于 Axes3D 类）是通过将 projection="3d" 关键字参数传递给 Figure.add_subplot 来创建的：
        #ax = fig.add_subplot(projection='3d') # 111 223
        n_points = self.n_points # 通过self.n_points传值
        x,y,z = n_points[:, 0, :], n_points[:, 1, :], n_points[:, 2, :]
        ax.scatter(x, y, z, c='blue', marker='o', s = 0.8)
        ax.axis('auto') 
        # 添加坐标轴的顺序是Z,Y,X
        ax.set_zlabel('Z')  # 坐标轴
        ax.set_ylabel('Y')
    ...
    def test(self, model, point_set, num_class=40, vote_num=1):
        ...
        print('pred: ',pred) #tensor形式的每个类别的预测值
        # 对预测结果每行取最大值得到分类
    ...
    def button_check_click(self):
        ...
        n_points = self.n_points # 通过self.n_points传值
        fig = self.draw(n_points[:, 0, :], n_points[:, 1, :], n_points[:, 2, :])
        # 创建一个FigureCanvas对象，并将Figure对象传入
        canvas = FigureCanvas(fig)
        # 获取pixmap对象
        pix_img = QPixmap(canvas.grab().toImage())
        ...
        pix_img = pix_img.scaled(2000, 2000, Qt.KeepAspectRatio)
        # 在label控件上显示选择的图片
        self.label3.setPixmap(pix_img)
        self.label3.setScaledContents(True)#设置pix_img填充满Label
    ...
    def draw(self, x, y, z): 
        ...
        # 3D 轴（属于 Axes3D 类）是通过将 projection="3d" 关键字参数传递给 Figure.add_subplot 来创建的：
        #ax = fig.add_subplot(projection='3d') # 111 223
        ax.scatter(x, y, z, c='blue', marker='o', s = 0.1)
        ax.axis('auto') 
        # 添加坐标轴的顺序是Z,Y,X
        ax.set_zlabel('Z')  # 坐标轴
        ax.set_ylabel('Y')
        ax.set_xlabel('X')
        #ax.set_title('三维点云物件图')
...
class Child2Window(QDialog):
    ...
    def init_label(self):
        label_layout = QHBoxLayout() # 创建标签对象
        label_layout.addStretch(1)
        label1 = QLabel('分割结果预测图')
        label_layout.addWidget(label1)
        label_layout.addStretch(2)
        label2 = QLabel('原始点云分割图')
    ...
    def button_check_click(self):# 先分割后保存
    ...
    def generate_predict(self):
        if self.dataset_name == 'shapenetcore_partanno_segmentation_benchmark_v0_normal':
            for batch_id, (points, label, target) in tqdm.tqdm(enumerate(self.testDataLoader),
                ...
                # print("points.shape: ", points.shape) # point.shape: torch.Size([1, 2500, 6])
